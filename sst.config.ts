...
export default $config({
  app(input) {
    return {
      ...
      home: "aws",
      providers: { neon: "0.13.0", cloudflare: "6.13.1" },
    };
  },
  async run() {
    const domain = 
      $app.stage === "production" ? "bexbox-padel.com" : "dev.bexbox-padel.com";
    ...
    if ($app.stage === "production") {
      ...
      db = new sst.Linkable("NeonDB", {
        properties: {
          connectionString:
            "postgresql://admin:admin@localhost:5432/mydb?sslmode=disable",
        },
      });
    }
